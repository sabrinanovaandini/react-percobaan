...
export default function VisiMisiTujuan() {
  ...
  const [misiData, setMisiData] = useState<ItemData[]>([
  ...
  const [tujuanData, setTujuanData] = useState<ItemData[]>([
  ...
  const handleDeleteClick = (item: ItemData, type: string) => {
    ...
    <main className="main">
      ...
      <div className="content-title">
        ...
        <div className="icon-table">
          ...
          <DialogMisi isOpen={showMisiPopup} onClose={() => setShowMisiPopup(false)} onSubmit={(nama: any) => {
            console.log("Data ditambahkan:", nama);
            setShowMisiPopup(false);
          }} />
          <DeletePopup isOpen={showDeletePopup} onClose={() => setShowDeletePopup(false)} onConfirm={() => {
            console.log("Data dihapus");
            setShowDeletePopup(false);
          }} />
        </div>
      </div>
      <Table columns={columns} data={misiDataWithIcons} onRowClick={() => {}} />
      ...
      <div className="content-title">
        ...
        <div className="icon-table">
          ...
          <FontAwesomeIcon icon={faFileExcel} style={{ color: "green", cursor: "pointer" }} />
          <FontAwesomeIcon icon={faArrowsRotate} style={{ color: "blue", cursor: "pointer" }} />
          <DialogTujuan isOpen={showTujuanPopup} onClose={() => setShowTujuanPopup(false)} onSubmit={(nama: any) => {
            console.log("Data ditambahkan:", nama);
            setShowTujuanPopup(false);
          }} />
        </div>
      </div>
      <Table columns={columns2} data={tujuanDataWithIcons} onRowClick={() => {}} />

      <PopupDelete
        show={showPopup}
        label={selectedItem?.type === "misi" ? "Misi" : "Tujuan"}
        value={selectedItem?.keterangan || ""}  // Default to an empty string if undefined
        onCancel={closePopup}
